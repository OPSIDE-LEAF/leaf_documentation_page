...
| **Ruta local tipada** | Modelo de invocación vigente: el host llama capabilities con referencias Kotlin tipadas; los errores de tipo fallan al compilar. |
...
| **Terminalidad única** | Garantía de que una sesión produce exactamente un resultado terminal (`Completed`, `Cancelled` o `Failed`); los eventos tardíos se rechazan. |
...
| **Fallo técnico** | Error inesperado normalizado a `LeafException` (redactada: expone `moduleInfo`; la operación es interna) o a una razón técnica de sesión. |
